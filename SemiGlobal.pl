...
for(my $i=0;$i<@sequence1;$i++){ #vertical
	for(my $j=0;$j<@sequence2;$j++){ #horizontal
		...
		if($i==@sequence1-1 or $j==@sequence2-1){	#last row or column
			
			if($matrixValues[$i][$j][2]>=$max){
				
				$max=$matrixValues[$i][$j][2];
				$maxi=$i;
				$maxj=$j;
...
while($maxi<@sequence1-1){
		$maxi++;
		unshift @newSeq2,"-";
		unshift @newSeq1,$sequence1[$maxi];
		}
		
	while($maxj<@sequence2-1){
		$maxj++;
		unshift @newSeq1,"-";
		unshift @newSeq2,$sequence2[$maxj];
		}
...
foreach my $seq (@newSeq2){
	...
	print "\nOptimal Score: $matrixValues[$maximumI][$maximumJ][2]"."\n";